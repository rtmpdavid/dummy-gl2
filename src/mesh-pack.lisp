...
(defmethod mesh-pack-add (mesh-pack mesh)
  (when (and (mesh-pack mesh)
	     (find mesh (mesh-pack-meshes (mesh-pack mesh))))
    (mesh-pack-remove (mesh-verts mesh) mesh))
  (vector-push-extend mesh (mesh-pack-meshes mesh-pack))
  (incf (mesh-pack-length-verts mesh-pack) (* (layout-size (mesh-layout mesh))
					      (length (mesh-verts mesh))))
  (incf (mesh-pack-length-elts mesh-pack) (* (c-sizeof :unsigned-int)
...
(defmethod mesh-pack-remove (mesh-pack mesh)
  (when (find mesh (mesh-pack-meshes mesh-pack))
    (let ((vao (mesh-vao mesh)))
      (decf (mesh-pack-length-verts mesh-pack) (* (layout-size (mesh-layout mesh))
						  (length (mesh-verts mesh))))
      (decf (mesh-pack-length-elts mesh-pack) (* (c-sizeof :unsigned-int)
						 (length (mesh-elts mesh))))
      (setf (mesh-pack-validp mesh-pack) nil
...
(defun mesh-pack-fill-ebo (pack)
  (let ((ebo (mesh-pack-ebo pack)))
    (xbo-alloc ebo :unsigned-int (mesh-pack-length-verts pack))
    (loop for mesh across (mesh-pack-meshes pack)
	  with offset = 0
